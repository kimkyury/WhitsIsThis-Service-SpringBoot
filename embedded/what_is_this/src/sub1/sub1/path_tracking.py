    ...
    def timer_callback(self):
        ...
        if self.is_status and self.is_odom ==True and self.is_path==True:
            ...
            if len(self.path_msg.poses)> 1:
                ...
                self.lfd=(lateral_error+self.status_msg.twist.linear.x)*0.5
                ...
                if self.is_look_forward_point :
                    ...
                    '''
                    # out_vel = (exp(self.lfd/2.5))*exp((-1)*(theta)/(2*sqrt(2)))  # Use the robot's current linear velocity
                    # out_rad_vel = pow(abs(theta),2)/(2*pi)+0.1*theta if theta >= 0 else -((pow(abs(theta),2)/(2*pi))+0.1*theta)  # Adjust this factor as needed

                    out_vel = 1*exp((-1)*(abs(theta))/(2*sqrt(2))) 
                    out_rad_vel = 2*theta

                    print(f"le : {round(lateral_error,3)}, v : {round(self.status_msg.twist.linear.x,3)}, yaw : {round(self.robot_yaw,3)}, lfs : {self.lfd}, theta : {round(theta,3)}, out_vel : {round(out_vel,3)}, out_rad_vel : {round(out_rad_vel,3)}")
